    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card" id="invoiceList">
                ...
                @endif
                <form action="{{ route('branding.update.routePlans', $routePlan->fld_rpid) }}" method="POST" name="add_new">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="live-preview">
                            <input type="hidden" value="{{$routePlan->fld_pid}}" name="fld_project_id">
                            <div class="row">
                                <!--end col-->
                                ...
                                <label for="state_id" class="form-control-label">Towns<sup>*</sup></label>
                                    <select name="town_id" id="town_id"
                                        class="form-control bg-light border-light flatpickr-input" required>
                                        <option value="">@lang('All Towns') </option>
                                        @foreach($towns as $town)
                                        <option value="{{$town->fld_tid}}" @selected($routePlan->fld_town_id == $town->fld_tid)>{{$town->fld_town}} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-4 mb-3">
                                <label for="state_id" class="form-control-label">Outlets<sup>*</sup></label>
                                    <select name="outlet_id" id="town_id"
                                        class="form-control bg-light border-light flatpickr-input" required>
                                        <option value="">@lang('All Outlets') </option>
                                        @foreach($outlets as $outlet)
                                        <option value="{{$outlet->fld_oid}}" @selected($routePlan->fld_outlet_id == $outlet->fld_oid)>{{$outlet->fld_outlet}} </option>
                                        @endforeach
                                    </select>
                                </div>
