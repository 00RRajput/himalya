...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('titleHeading')
        Complete Installation ( Total : {{ count($records) }} )
        @endslot
        @slot('title')
            Recce Approve
        @endslot
        ...
        <div class="col-lg-12">
            <div class="card" id="invoiceList">
                ...
                <div class="card-body bg-soft-light border border-dashed border-start-0 border-end-0">
                    <form action="{{ url('/branding/installation-completed') }}" method="GET">
                        <div class="row g-3">
                            <!--end col-->
                            <div class="col">
                                <select name="project_id" id="project_id"
                                    ...
                                    @foreach ($projects as $project)
                                        @if($project->fld_pid==4)<option value="{{ $project->fld_pid }}" @selected(session('project_id') == $project->fld_pid || 4 == $project->fld_pid)>
                                    ...
                                    @forelse ($records as $key=> $record)
                                        <tr>
                                           ...
                                           {{-- <td class="td">
                                            ...
                                            <td class="td">
                                            @if ($record->fld_photo_file_install)
                                                    <a class="image-popup"
                                                        href="{{ env('SERVER_BASE_URL') }}{{ $record->fld_photo_path_install }}/{{ $record->fld_photo_file_install }}"
                                                        title="{{ $record->fld_photo_file_install }}">
                                                        <img class="gallery-img img-fluid mx-auto avatar-xs rounded me-2"
                                                            src="{{ env('SERVER_BASE_URL') }}{{ $record->fld_photo_path_install }}/{{ $record->fld_photo_file_install }}"
                                                            alt="{{ $record->fld_photo_file_install }}">
                                                    </a>
                                                @else
                                                    <i class="bx bx-image bx-md"></i>
                                                @endif
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ route('branding.images', ['id'=> $record->fld_raid, 'type'=> 'install']) }}" style="font-size:11px;">View All Images</a>
                                                @endif
                                            </td>
                                           
                                            <td class="td">
                                                @if ($record->fld_install_date)
                                                {{ date(env('DATE_FORMAT'), strtotime($record->fld_install_date)) }}
                                                @else
                                                -
                                                @endif
                                            </td>
                                            @if (in_array(Auth::user()->fld_role, [1, 3]))
                                               <td> 
                                                    <div class="d-flex gap-2">
                                                    <a href="{{ route('branding.installation.approve', $record->fld_raid) }}" onclick="return confirm('Are you sure you want to approve?')"><button type="button" class="btn btn-success">Approve</button></a>
                                                    <a href="{{ route('branding.installation.reject', $record->fld_raid) }}" onclick="return confirm('Are you sure you want to reject?')"><button type="button" class="btn btn-danger">Reject</button></a>
