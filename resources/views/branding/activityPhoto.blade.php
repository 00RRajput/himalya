                ...
                <div class="card-body">
                    <div>
                        <div class="row mt-1" id="seller-list">
                            <div class="col-lg-3">
                                @if($images[0]->fld_photo_path_recce)
                                            <a class="image-popup m-0 p-0"
                                                href="{{ env('SERVER_BASE_URL') }}{{ $images[0]->fld_photo_path_recce }}/{{ $images[0]->fld_photo_file_recce }}"
                                                title="">
                                                <img class="gallery-img img-fluid mx-auto"
                                                    src="{{ env('SERVER_BASE_URL') }}{{ $images[0]->fld_photo_path_recce }}/{{ $images[0]->fld_photo_file_recce }}"
                                                    alt="">
                                                <div class="gallery-overlay">
                                                    {{--<h5 class="overlay-caption">{{ $image->photo_type ?? '' }}</h5>--}}
                                                </div>
                                            </a>
                                            @endif
                                           {{-- <p class="text-muted m-0 p-0">
                                                {{ date(env('DATE_FORMAT'), strtotime($image->fld_created_at)) }}
                                                <a href="http://maps.google.com/?q={{ $image->fld_lat }},{{ $image->fld_long }}"
                                                    target="_blank" class="ri-map-pin-fill"></a>

                                            </p>
                                            <p class="m-0 p-0 text-muted">{{ $image->user_name }} |
                                                {{ $image->fld_state ?? 'NA' }}</p>
                                            <p class="text-muted">{{ $image->project_name ?? '' }}</p>--}}
