        ...
        <div class="col-lg-12">
            <div class="card" id="invoiceList">
                ...
                <div class="card-body  bg-soft-light border border-dashed border-start-0 border-end-0">
                    <form action="{{ route('mandi.routePlan.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <!--end col-->
                            <div class="row">
                                <div class="col-4">
                                    <select id="project_id" name="project_id"
                                        ...
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->fld_pid }}" @if($project->fld_pid == 2) selected @endif> {{ $project->fld_name }} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-4">
                                ...
                                <tbody class="list form-check-all" id="">
                                    @forelse ($records as $key => $record)
                                        <tr>
                                            ...
                                            <td class="td">
                                                <a href="/{{ $record->fld_path }}">{{ $record->fld_file }}</a>
                                            </td>

                                            <td class="td">
                                                {{ date(env('DATE_FORMAT', 'd-m-Y'), strtotime($record->fld_updated_at)) }}
                                            </td>
