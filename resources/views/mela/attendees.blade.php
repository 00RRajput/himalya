...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('titleHeading')
            Mela Attendees (Total: {{ $records->total() }} )
        @endslot
        @slot('title')
            @lang('Attendeess')
        @endslot
        ...
        <div class="col-lg-12">
            <div class="card" id="invoiceList">
                ...
                <div class="card-body bg-soft-light border border-dashed border-start-0 border-end-0">
                    <form action="{{ url('mela/attendees') }}" method="GET">
                        <div class="row g-3">
                            <!--end col-->
                            <div class="col">
                            ...
                            <div class="col">
                                <div class="d-flex gap-2">
                                    ...
                                    <a class="btn btn-secondary col ml-1"
                                        href="{{ url('mandit/consumer-sales') }}">Reset</a>
                                ...
                                <tbody class="list form-check-all" id="">
                                    @forelse ($records as $key=> $record)
                                        <tr>
                                            ...
                                            <td>{{ $record->fld_phone }}</td>
                                            <td>{{ $record->fld_village }}</td>
                                            <td>{{ $record->s_count }}</td>
                                            <td>@include('components.locationTD')</td>
                                            <td>
                                                <div class="d-flex gap-2">

                                                  {{--  <a class=""
                                                        href="{{ route('mela.consumersales.details', $record->fld_mcsid) }}">
                                                        <i class="ri-eye-line"></i></a>
                                                    @if (in_array(Auth::user()->fld_role, [1, 3]))
                                                        <form onsubmit="return confirm('Do you really want to delete?');"
                                                            action="{{ route('mela.consumerSales.delete', $record->fld_mcsid) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
