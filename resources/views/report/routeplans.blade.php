                ...
                <div class="p-3">
                    <div class="gap-2 d-flex justify-content-end ">
                        @if (in_array(Auth::user()->fld_role, [1]))
                            <a class="btn  btn-success btn-sm" href="{{ url('master/route-plans') }}"
                        ...
                        <a class="btn  btn-primary btn-sm" href="{{ route('master.create.routePlans') }}"
                                ...
                                <tbody class="list form-check-all" id="">
                                    @forelse ($records as $key => $record)
                                        <tr>
                                            ...
                                            <td class="td">{{ $record->attenUser->fld_name ?? 'NA' }}</td>
                                            <td class="td">{{ $record->village_name }}</td>
                                            <td class="td">{{ $record->district_name }}</td>
                                            <td class="td">{{ $record->state_name }}</td>
                                            <td class="td">{{ $record->project->fld_name }}</td>
                                            <td class="td">
                                                <div class="d-flex gap-2">

                                                    <a class=""
                                                        href="{{ route('master.edit.routePlans', $record->fld_rpid) }}">
                                                        <i class="ri-edit-line"></i></a>
                                                    @if (in_array(Auth::user()->fld_role, [1, 3]))
                                                        <form onsubmit="return confirm('Do you really want to delete?');"
                                                            action="{{ route('master.delete.routePlans', $record->fld_rpid) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
