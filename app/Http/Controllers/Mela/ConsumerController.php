...
class ConsumerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query =  DB::table('mela_consumers')
            ->select(
                'mela_consumers.fld_mcid',
                'mela_consumers.fld_pid',
                'mela_consumers.fld_uid',
                'mela_consumers.fld_name as consumer_name',
                'mela_consumers.fld_phone as fld_number',
                'mela_consumers.fld_remark',
                'mela_consumers.fld_lat',
                'mela_consumers.fld_long',
                'mela_consumers.fld_created_at',
                'mela_consumers.fld_village',
                'projects.fld_name as project_name',
                "users.fld_name",
                "mela_consumer_sales.fld_total"
            )

            ->join('users', 'users.fld_uid', '=', 'mela_consumers.fld_uid', 'left')
            ->join('mela_consumer_sales', 'mela_consumer_sales.fld_mcid', '=', 'mela_consumers.fld_mcid', 'left')
            ->join('projects', 'projects.fld_pid', '=', 'mela_consumers.fld_pid');

        if ($project_id) {
            $query->where('mela_consumers.fld_pid', $project_id);
        ...
        if ($request->user_id) {
            $query->where('mela_consumers.fld_uid', $request->user_id);
        }
        ...
        if ($request->start_date) {
            $query->where('mela_consumers.fld_created_at', '>=', $request->start_date);
        }
        if ($request->end_date) {
            $query->where('mela_consumers.fld_created_at', '<=', $request->end_date);
        }

        // if ($request->district_id) {
        //     $query->where('mela_consumers.fld_district_id', $request->district_id);
        // }
        if ($retailer_id) {
            $query->where('mela_consumers.fld_rid', $retailer_id);
        }
        ...
        // dd($records);
        return view('mela.consumer.index', compact('records', 'projects', 'states', 'districts', 'users'));
    }
    /**
    ...
    public function addNewOrder(Request $request)
    {
        ...
        $consumer = DB::table('mela_consumers')->where('fld_cid', $request->cid)->first();

        return view('mela.consumer.addNewOrder', compact('products', 'consumer'));
    }
    ...
    public function storeNewOrder(Request $request)
    {
        ...
        try {
           ...
           $consumer = DB::table('mela_consumers')->where('fld_cid', $request->fld_cid)->first();
    ...
    public function destroy($consumer)
    {
        ...
        DB::table('mela_consumers')->where('fld_mcid', $consumer)->first();
        // Delete user along with related orders and order items
        $sales = DB::table('mela_consumer_sales')->where('fld_mcid', $consumer)->get();
        foreach ($sales as $order) {
            DB::table('mela_consumer_sales_items')->where('fld_mcsid', $order->fld_mcsid)->delete();
            
        }
        DB::table('mela_consumer_sales')->where('fld_mcid', $consumer)->delete();
        DB::table('mela_consumers')->where('fld_mcid', $consumer)->delete();
       
        return redirect()->route('mela.consumer')
