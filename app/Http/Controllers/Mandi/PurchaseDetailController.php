...
class PurchaseDetailController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = DB::table('purchases')->select(
            ...
            ->where('purchases.fld_project_id', '=', 2);
        ...
        return view('mandi.purchasedetail', compact('records', 'projects', 'users', 'states', 'total_order_value'));
    }
    ...
    public function getStockists(Request $request)
    {
        ...
        return view('mandi.stockists', compact('records', 'projects', 'users', 'states', 'districts'));
    }
    ...
    public function addNewStockistOrder(Request $request)
    {
        ...
        return view('mandi.stockiestAddNewOrder', compact('products', 'stockist'));
    }
    ...
    public function getPurchaseDetails($id)
    {
        ...
        return view('mandi.purchasedetailItems', compact('records', 'products', 'purchases'));
    }
    ...
    public function activityPhoto(Request $request)
    {
        ...
        $query = ActivityPhoto::with('user')
            ...
            ->rightJoin('x_districts', 'x_districts.fld_did', '=', 'activity_photos.fld_did', 'left')
            ->rightJoin('projects', 'projects.fld_pid', '=', 'activity_photos.fld_pid', 'left')
            ->rightJoin('x_ptypes', 'x_ptypes.fld_ptid', '=', 'activity_photos.fld_ptid', 'left')
            ->where('activity_photos.fld_pid', 2);
        ...
        if ($project_id) {
            $query->where('activity_photos.fld_pid', $project_id);
            $request->session()->put('project_id', $project_id);
            $states = $this->getStateByProjectID($project_id);
        ...
        if ($request->user_id) {
            $query->where('activity_photos.fld_uid', $request->user_id);
        }

        if ($request->state_id) {
            $query->where('activity_photos.fld_state_id', $request->state_id);
            $districts =  $this->getDistrictByStateID($request->state_id);
        }

        if ($request->district_id) {
            $query->where('activity_photos.fld_did', $request->district_id);
        }

        if ($request->start_date) {
            $query->whereDate('activity_photos.fld_date', '>=', $request->start_date);
        }
        if ($request->end_date) {
            $query->whereDate('activity_photos.fld_date', '<=', $request->end_date);
        }

        $records =  $query->orderby('activity_photos.fld_date', 'desc')->paginate(20);
        // dd($records);
        return view('mandi.activityPhoto', compact('records', 'projects', 'users', 'states', 'districts'));
    }
